...
`mihomo-manager-webui` 是一个独立的 Mihomo Web 管理面板。它不依赖额外的管理脚本文件，支持本地管理和远端管理两种模式，并使用内置的白名单动作管理 Mihomo、订阅配置、内核代理入口、系统代理、systemd 服务和日志。
...
- Mihomo 内核代理入口配置，支持 HTTP、SOCKS5、Mixed、Redir、TProxy 和 TUN
- 代理模式和端口可视化调整，支持 Rule / Global / Direct
- 系统代理配置，基于当前 HTTP/Mixed 入口支持新 shell 和 apt
- HTTP/Mixed 和 SOCKS5/Mixed 连通性测试
- Mihomo 和订阅更新日志查看
- 简体中文 / English 输出设置
...
1. 添加订阅链接，保存订阅名称、描述和 User-Agent。
2. 更新订阅后读取 Mihomo 代理组，逐条测试节点延迟并选择合适节点。
3. 使用 Mihomo 内核提供的 HTTP、SOCKS5、Mixed、Redir、TProxy 或 TUN 入口进行代理。

### Proxychains 评估

Linux 可以不依赖 proxychains 直接使用 Mihomo 内核代理：支持代理环境变量的程序可以走 HTTP/Mixed 入口，支持 SOCKS 的程序可以走 SOCKS5/Mixed 入口；需要透明代理时，可以使用 Redir、TProxy 或 TUN。proxychains 的价值主要是让少数不支持代理参数或环境变量的命令行程序强制走 SOCKS，因此它不再是本项目主流程依赖。
...
- 系统：调整 Mihomo 内核代理方式和端口，开启/关闭系统环境代理
...
`mihomo-manager-webui` is a standalone Web UI for managing a Mihomo proxy setup. It does not depend on an external management script file. It supports both local management and remote management, and uses built-in whitelisted actions to manage Mihomo, subscriptions, core proxy inbounds, system proxy settings, systemd services, and logs.
...
- Mihomo core inbound configuration for HTTP, SOCKS5, Mixed, Redir, TProxy, and TUN
- Visual proxy mode and port editing for Rule / Global / Direct
- System proxy helpers for new shells and apt, based on the current HTTP/Mixed inbound
- HTTP/Mixed and SOCKS5/Mixed connectivity tests
...
1. Add a subscription URL with its name, description, and User-Agent.
2. Update the subscription, inspect Mihomo proxy groups, test node latency progressively, and choose a suitable node.
3. Use Mihomo core inbounds such as HTTP, SOCKS5, Mixed, Redir, TProxy, or TUN for proxying.

### Proxychains Assessment

Linux can use Mihomo directly without proxychains. Programs that support proxy environment variables can use the HTTP/Mixed inbound, programs that support SOCKS can use SOCKS5/Mixed, and transparent proxy scenarios can use Redir, TProxy, or TUN. proxychains is mainly useful for a small set of CLI programs that do not support proxy arguments or environment variables, so it is no longer required by the main product workflow.
...
- Dashboard: service, subscription, system proxy, and core inbound status
...
- System: adjust Mihomo core proxy modes and ports, enable/disable system environment proxy
